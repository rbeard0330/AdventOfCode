...
class Segment:
    ...
    @property
    def length(self):
        return manhattan_distance(self.p1, self.p2)
...
class Wire:
    ...
    def _process_input(self, line):
        # First point is off the origin by 1 to avoid false matches.
    ...
    def find_closest_intersect(self, other):
        intersect_list = []
        for my_counter, my_seg in enumerate(self.segment_list):
            for other_counter, other_seg in enumerate(other.segment_list):
                if point := intersect(my_seg, other_seg):
                    my_dist = (
                        sum(
                            [s.length
                             for s in self.segment_list[:my_counter]])
                        + manhattan_distance(my_seg.p1, point)
                        + 1)    # Adding back origin
                    other_dist = (
                        sum(
                            [s.length
                             for s in other.segment_list[:other_counter]])
                        + manhattan_distance(other_seg.p1, point)
                        + 1)    # Adding back origin
                    intersect_list.append((point, my_dist + other_dist))
...
print(min([p[1] for p in intersect_list]))
